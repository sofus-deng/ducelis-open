    ...
    <main className="mx-auto flex min-h-screen w-full max-w-6xl flex-col px-6 py-10 sm:px-10 lg:px-12">
      <section className="rounded-3xl border border-white/10 bg-[var(--panel)] px-6 py-14 shadow-[0_24px_80px_rgba(15,23,42,0.18)] sm:px-10 lg:px-14">
        ...
        <div className="mt-5 max-w-3xl space-y-4">
          ...
          <p className="max-w-2xl text-lg leading-8 text-cyan-50/90 sm:text-xl">
      ...
      <section className="mt-10 grid gap-6 md:grid-cols-2">
        {scenarios.map((scenario) => (
          <article
            ...
            className="rounded-3xl border border-black/5 bg-white/80 p-7 shadow-[0_16px_48px_rgba(15,23,42,0.08)] backdrop-blur"
          >
            <div className="flex items-center justify-between gap-4">
              <p className="text-sm font-semibold uppercase tracking-[0.2em] text-[var(--accent)]">
                {scenario.difficultyLabel}
              </p>
              <span className="rounded-full bg-[var(--panel-subtle)] px-3 py-1 text-sm text-[var(--muted)]">
                3 review points
              </span>
            </div>
            ...
            <Link
              ...
              className="mt-6 inline-flex items-center rounded-full bg-[var(--foreground)] px-5 py-3 text-sm font-semibold text-white transition hover:-translate-y-0.5"
            >
              Open scenario
            </Link>
          </article>
